...
public class PaymentService implements CreatePaymentUseCase {
    ...
    public Payment createPayment(CreatePaymentRequestDTO requestDTO) {
        Payment newPayment = new Payment(null, requestDTO.amount(), requestDTO.name(), requestDTO.description(), requestDTO.startDate(), requestDTO.startDate(), requestDTO.endDate(), requestDTO.periodDays(), PaymentStatus.ACTIVE);
